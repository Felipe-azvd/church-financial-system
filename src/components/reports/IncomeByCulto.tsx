...
export default function IncomeByCulto({
  ...
  return (
    <div className="card w-full mb-8 overflow-hidden">
      ...
      <div className="overflow-x-auto md:overflow-visible" style={{ maxHeight: '420px', overflowY: 'auto' }}>
          ...
          {!data || data.length === 0 ? (
            ...
            data.map((item) => (
              <tr key={item.culto} className="flex flex-col mb-4 border border-white/10 rounded-xl p-4 bg-white/[0.02] md:bg-transparent shadow-sm md:table-row md:mb-0 md:border-b md:border-white/5 md:p-0 md:shadow-none hover:bg-white/[0.04] md:hover:bg-white/5 transition-colors">
                ...
                <td className="flex justify-between items-center py-2 border-b border-white/5 last:border-b-0 md:table-cell md:border-none md:py-4 md:text-right font-bold text-[var(--success)]">
