    ...
    <>
      ...
      <aside className={`sidebar-glass flex flex-col flex-shrink-0 h-full overflow-hidden transition-all duration-300 z-50 fixed md:relative top-0 left-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} ${isCollapsed ? 'md:w-20 md:px-3' : 'w-64 px-5'}`} style={{ paddingBottom: '2rem', paddingTop: '2rem' }}>
        ...
        <div className={`mb-10 flex items-center h-8 transition-all duration-300 ${isCollapsed ? 'md:justify-center' : 'justify-between'}`}>
          <img src="/logo-c.png" alt="Logo" className={`h-auto object-contain transition-all duration-300 ${isCollapsed ? 'md:w-0 md:opacity-0' : 'w-[160px] opacity-100'}`} />
        ...
        <nav className="flex flex-col gap-2 flex-1 overflow-x-hidden overflow-y-auto no-scrollbar">
          {activeNavItems.map((item) => {
            ...
            if (item.label === 'Configurações') {
              return (
                <div key={item.href} className="flex flex-col">
                  <button onClick={() => { if (isCollapsed) setIsCollapsed(false); setIsConfigOpen(!isConfigOpen); }} className={`nav-item flex items-center justify-between rounded-lg transition-colors duration-200 ${pathname.startsWith('/configuracoes') && !isConfigOpen ? 'bg-white/5 text-white' : 'text-[var(--text-muted)] hover:text-white hover:bg-white/5'} ${isCollapsed ? 'md:justify-center p-3' : 'px-4 py-3'}`}>
            ...
            if (item.label === 'Auditoria') {
              return (
                <div key={item.href} className="flex flex-col">
                  <button onClick={() => { if (isCollapsed) setIsCollapsed(false); setIsAuditoriaOpen(!isAuditoriaOpen); }} className={`nav-item flex items-center justify-between rounded-lg transition-colors duration-200 ${pathname.startsWith('/super-admin/auditoria') && !isAuditoriaOpen ? 'bg-amber-500/10 text-amber-500 font-medium' : 'text-[var(--text-muted)] hover:text-white hover:bg-white/5'} ${isCollapsed ? 'md:justify-center p-3' : 'px-4 py-3'}`}>
                    <div className="flex items-center">
                  ...
                  <div className={`overflow-hidden transition-all duration-300 flex flex-col gap-1 ${isAuditoriaOpen && !isCollapsed ? 'max-h-40 mt-1 opacity-100' : 'max-h-0 opacity-0'}`}>
                    <Link href="/super-admin/auditoria/clientes" onClick={handleLinkClick} className={`pl-12 py-2 text-sm rounded-lg ${pathname.includes('/auditoria/clientes') ? 'text-amber-500 bg-amber-500/10 font-medium' : 'text-[var(--text-muted)] hover:text-white'}`}>Auditoria Clientes</Link>
                    <Link href="/super-admin/auditoria/master" onClick={handleLinkClick} className={`pl-12 py-2 text-sm rounded-lg ${pathname.includes('/auditoria/master') || pathname === '/super-admin/auditoria' ? 'text-amber-500 bg-amber-500/10 font-medium' : 'text-[var(--text-muted)] hover:text-white'}`}>Auditoria Super Admin</Link>
                  </div>
                </div>
              )
            }
            ...
            return (
              <Link key={item.href} href={item.href} onClick={handleLinkClick} className={`nav-item flex items-center rounded-lg transition-all duration-200 ${isActive ? (isMasterArea ? 'bg-amber-500/10 text-amber-500 font-medium' : 'bg-white/10 text-white font-medium') : 'text-[var(--text-muted)] hover:text-white hover:bg-white/5'} ${isCollapsed ? 'md:justify-center p-3' : 'px-4 py-3'}`}>
        ...
        <div className="mt-auto flex flex-col gap-2">
          ...
          <div className={`flex items-center ${isCollapsed ? 'flex-col gap-2' : 'flex-row justify-between'}`}>
            ...
            <button 
              ...
              className={`p-2 rounded-lg text-[var(--text-muted)] hover:text-white hover:bg-white/10 transition-colors flex-shrink-0 ${isCollapsed ? '' : 'ml-2'}`}
              ...
              <div className="flex flex-col flex-1 min-w-0 pr-2 relative">
                ...
                {churchNetwork.length > 1 ? (
                  <>
                    ...
                    >
                      <span className="text-[10px] font-medium text-[var(--text-muted)] truncate uppercase tracking-tighter group-hover:text-white transition-colors">
                        {churchName}
                      </span>
                      <ChevronsUpDown className="w-3 h-3 text-[var(--text-muted)] flex-shrink-0 group-hover:text-white transition-colors" />
                    </button>
                    
                    {/* DROPDOWN GLASSMORPHISM */}
                    {isNetworkOpen && (
                      <div className="absolute bottom-full left-0 mb-2 w-56 rounded-xl border border-white/10 bg-[#0a1511]/95 backdrop-blur-xl shadow-2xl z-[100] animate-[fadeIn_0.2s_ease-out] p-2">
                        <div className="text-[10px] uppercase font-bold tracking-widest text-[var(--text-muted)] mb-2 px-2 pt-1 border-b border-white/5 pb-2">
                          Selecione o Tenant
                        </div>
                  ...
                  <span className="text-[10px] mt-0.5 font-medium text-[var(--text-muted)] truncate uppercase tracking-tighter">
                    {churchName}
                  </span>
                )}
              </div>
